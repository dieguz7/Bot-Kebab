    ...
    async execute(interaction) {
        ...
        if (!role1 || !role2 || !role3) {
            return interaction.reply({ 
                content: `❌ Errore: Crea i ruoli "${rName1}", "${rName2}" e "${rName3}" su Discord.`, 
                ephemeral: true 
            });
        }
        ...
        try {
            if (target.roles.cache.has(role3.id)) {
                return interaction.reply({ content: `⚠️ ${target} ha già 3/3 richiami.`, ephemeral: true });
            ...
            await target.send(`Hai ricevuto un richiamo su **${interaction.guild.name}** (${statoWarn}).`).catch(() => {});

        } catch (error) {
            await interaction.reply({ content: "❌ Errore permessi: sposta il ruolo del bot in alto!", ephemeral: true });
        }
    },
};
